...
describe('RAGService', () => {
  ...
  beforeEach(async () => {
    ...
    mockChromaCollectionGet = jest.fn().mockResolvedValue({ documents: [], ids: [], metadatas: [] });
    mockChromaCollectionDelete = jest.fn().mockResolvedValue(undefined);
    mockChromaCollectionQuery = jest
      .fn()
      .mockResolvedValue({ distances: [[]], documents: [[]], ids: [[]], metadatas: [[]] });
  ...
  describe('queryChunks', () => {
    it('should generate query embedding via llmWrapper and call chromaCollection.query', async () => {
      ...
      const mockQueryResponse = {
        ...
        ids: [['id1', 'id2']],
        metadatas: [[{ text_chunk: 'RAG is...' }, { text_chunk: 'Retrieval Augmented Generation' }]], // ChromaDB might return full documents
      };
  ...
  describe('chunkMarkdown', () => {
    it('should split markdown into text chunks without original_content', async () => {
      // Made test async
